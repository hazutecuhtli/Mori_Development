...
def Manual_WikiSearch(concepts, synonyms, language = "es", languageSite = "es"):
    ...
    for consulta in tqdm(concepts, mininterval=50, total=len(concepts), desc="Processing", unit="row"):  
...
def SerpAPI_WikiSearch(consultas, keyword4search, chekpoints=100):
    ...
    for i, concepto in enumerate(consultas):
        ...
        wikipedia_link = get_wikipedia_link(new_concept)
        
        if not wikipedia_link:
            new_concept = translate_to_english(concepto) + " model"
            wikipedia_link = get_wikipedia_link(new_concept)      
        else:
            if ("archivo" in wikipedia_link.lower()) | ("pt" in wikipedia_link.lower()) | (wikipedia_link.split("/")[-1] == 'Machine_learning') | ("Contenido_por_wikiproyecto" in wikipedia_link):
                new_concept = translate_to_english(concepto) + " data machine_learning"        
                wikipedia_link = get_wikipedia_link(new_concept)               
        ...
        if wikipedia_link:
            ...
            if definicion:
                if detect(definicion) == "en":
                    definicion = translate_to_spanish(definicion)  
                pattern = r"\[\d+\]"
                definicion = re.sub(pattern, "", definicion)
                ...
                new_concept = translate_to_english(concepto) + " data"
                wikipedia_link = get_wikipedia_link(new_concept)        
...
def transformer_similarities(df_base, df_comp, thre=.2):
    ...
    for index, row in tqdm(df_comp.iterrows(), mininterval=100, total=df_comp.shape[0], desc="Processing", unit="row"):
...
def choosing_best_concept(df_base, df_a, df_b):
    ...
    for index, row in tqdm(df_a.iterrows(), mininterval=50, total=df_a.shape[0], desc="Processing", unit="row"):
        concept = row["concepto"]
        sen2 = row["descripcion"]
