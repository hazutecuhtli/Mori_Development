...
def Webscraping1(url):
    ...
    for index, row in tqdm(df.iterrows(), mininterval=50, total=df.shape[0], desc="Processing", unit="row"):
        conceptos.append(translation_function(row["concepto"], english=False))
        descripciones.append(translation_function(row["descripcion"], english=False))
...
def WebScraping2(url, synonyms):
    ...
    df.loc[:, 'source'] = [url.split("/")[2]]*len(concepts)
    print()
    for idx in tqdm(df.loc[df.index[df.concept.isna()]].index, mininterval=50, total=df.loc[df.index[df.concept.isna()]].index.shape[0], desc="Processing", unit="row"):
    #for idx in df.loc[df.index[df.concept.isna()]].index:
        df.loc[idx, 'concept'] = translation_function(df.loc[idx, 'concepto'], english=True)   
