...
from Notebooks.Data_Generation.DataWrangling_Functions import GeneratingData_WebScraping, GatheringData_PDFReader1
from Notebooks.Data_Generation.DataWrangling_Functions import GatheringData_PDFReader2, GatheringData_PDFReader3
from Notebooks.Data_Generation.DataWrangling_Functions import GatheringData_PDFReader4
from Notebooks.Data_Generation.DataCleaning_Functions import remove_parenthesis_complements
'''************************************************************************
Functions
************************************************************************'''
...
def main():
    ...
    path2save = os.path.join(sys.path[0], 'Notebooks','Data_Generation','Data', 'Results', 'Webscraping_Concepts.csv')
    print("Saving webscraped url concepts in: ", path2save)
    ...
    paths = []
    for book in books:
        paths.append(os.path.join(sys.path[0], 'Notebooks','Data_Generation','Books', book))

    df_pdf = GatheringData_PDFReader1(paths[0])
    df_pdf.to_csv(os.path.join(sys.path[0], 'Notebooks','Data_Generation', 'Data', 'Books', books[0].split('.')[0]+'.csv'))
    df_tmp = GatheringData_PDFReader2(paths[1])
    df_tmp.to_csv(os.path.join(sys.path[0], 'Notebooks','Data_Generation', 'Data', 'Books', books[1].split('.')[0]+'.csv'))
    df_pdf = pd.concat([df_pdf, df_tmp], ignore_index=True, sort=False)
    df_tmp = GatheringData_PDFReader3(paths[2])
    df_tmp.to_csv(os.path.join(sys.path[0], 'Notebooks','Data_Generation', 'Data', 'Books', books[2].split('.')[0]+'.csv'))
    df_pdf = pd.concat([df_pdf, df_tmp], ignore_index=True, sort=False)
    df_tmp = GatheringData_PDFReader4(paths[3])

    df_tmp.to_csv(os.path.join(sys.path[0], 'Notebooks','Data_Generation', 'Data', 'Books', books[3].split('.')[0]+'.csv'))
    df_pdf = pd.concat([df_pdf, df_tmp], ignore_index=True, sort=False)    


    path2save = os.path.join(sys.path[0], 'Notebooks','Data_Generation', 'Data', 'Results', 'AIBooks_Concepts.csv')
    ...
    print()
    for idx in tqdm(df_pdf.loc[df_pdf.index[df_pdf.concept.isna()]].index, mininterval=50, total=df_pdf.loc[df_pdf.index[df_pdf.concept.isna()]].index.shape[0], desc="Processing_Books&URLs", unit="row"):                              
    ...
    print("Saving webscraped books and url concepts in: ", path2save)    
    df_pdf.to_csv(path2save, index=False)


    path = os.path.join(sys.path[0], 'Notebooks','Data_Generation', 'Data', 'GPT')
    ...
    print("\n")
    path2save = os.path.join(sys.path[0], 'Notebooks','Data_Generation', 'Data', 'Results', 'GPT_Concepts.csv')
    ...
    for idx in tqdm(df_GPT.loc[df_GPT.index[df_GPT.concept.isna()]].index, mininterval=50, total=df_GPT.loc[df_GPT.index[df_GPT.concept.isna()]].index.shape[0], desc="Processing_GPT&Concepts", unit="row"):
    ...
    print("Saving webscraped GPT provided concepts in: ", path2save)        
    ...
    print("Saving concepts names in spanish and englihs in: ", path2save)          
